...
基线。考虑基于时序的模型,SLC-RF[70]和 FS-Net[39]是两个最先进的代表方法,分别  采用马尔可夫模型和循环神经网络。SLC-RF[70]使用消息类型和数据包长度马尔可夫模  型的输出概率作为特征,使用随机森林对加密流量进行分类。FS-Net[39]使用 GRU(Gated  Recurrent Unit)模型学习序列特征,对常见应用进行分类。由于 ERNN 的设计是以传统  的 LSTM 作为骨干架构,因此也考虑了正常的 LSTM 模型作为对比模型。
